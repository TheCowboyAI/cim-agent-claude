...
impl ConversationAggregate {
    ...
    pub fn from_command(
        ...
        match command {
            ...
            _ => Err(DomainError::InvalidCommand("Expected StartConversation command".to_string())),
        }
    }
    ...
    pub fn handle_command(
        ...
        match command {
            ...
            Command::EndConversation {
                ...
                let total_tokens_used = self.exchanges
                    .iter()
            ...
            _ => Err(DomainError::InvalidCommand("Unsupported command for existing conversation".to_string())),
        }
    }
    ...
    fn validate_rate_limit(&self) -> Result<(), DomainError> {
        ...
        let recent_prompts = self.exchanges
            .iter()
...
mod tests {
    ...
    fn test_send_prompt_command() {
        ...
        let events = aggregate.handle_command(send_prompt_cmd, CorrelationId::new()).unwrap();
